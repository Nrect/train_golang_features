...
import (
	...
	osEnv "testProject/packages/os/env"
)

const (
	Neo4jAuraUserName      = `Neo4jAuraUserName`
	Neo4jAuraPassword      = `Neo4jAuraPassword`
	Neo4jAuraConnectionURL = `Neo4jAuraConnectionURL`
)

type Neo4jAura struct {
	UserName      string `json:"user_name,omitempty"`
	Password      string `json:"password,omitempty"`
	ConnectionURL string `json:"connection_url,omitempty"`
}

func (n *Neo4jAura) GetAuthToken() neo4j.AuthToken {
	return neo4j.BasicAuth(
		n.UserName,
		n.Password,
		"")
}
...
func main() {
	neo4jAuraAuth := Neo4jAura{
		UserName:      osEnv.GetEnvValue(Neo4jAuraUserName),
		Password:      osEnv.GetEnvValue(Neo4jAuraPassword),
		ConnectionURL: osEnv.GetEnvValue(Neo4jAuraConnectionURL),
	}
	...
	driver, err := neo4j.NewDriverWithContext(neo4jAuraAuth.ConnectionURL, neo4jAuraAuth.GetAuthToken())
	if err != nil {
		panic(err)
	}
